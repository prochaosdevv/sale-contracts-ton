...
import { JettonMinter , jettonContentToCell} from '../wrappers/Jetton';
import { promptBool, promptAmount, promptAddress, displayContentCell, waitForTransaction } from '../wrappers/ui-utils';
// import { jettonContentToCell, NftCollection } from '../wrappers/NftCollection';
// import { jettonContentToCell, NftCollection } from '../wrappers/NftCollection';
let minterContract:OpenedContract<JettonMinter>;
// let nftminterContract:OpenedContract<NftCollection>;

// const adminActions  = ['Mint', 'Change admin', 'Mint NFT'];
const adminActions  = ['Mint Token','Info'];
...
const infoAction = async (provider:NetworkProvider, ui:UIProvider) => {
    // const jettonData = await nftminterContract.get_collection_data();
    ...
    ui.write(`Max supply:${fromNano(jettonData.maxSupply)}\n`);
...
const deployNFT = async (provider:NetworkProvider, ui:UIProvider) => {
    ...
    do {
        retry = false;
        const fallbackAddr =sender.address;
        mintAddress = await promptAddress(`Please specify address to mint to`, ui, fallbackAddr);
        mintAmount  = await promptAmount('Please provide amount:', ui);
        ui.write(`Mint ${mintAmount} index NFT to ${mintAddress}\n`);
        retry = !(await promptBool('Is it ok?(yes/no)', ['yes', 'no'], ui));
    } while(retry);

    ui.write(`Minting ${mintAmount} to ${mintAddress}\n`); 
     
    const curState     = await provider.api().getContractState(nftminterContract.address);

    if(curState.lastTransaction === null)
        throw("Last transaction can't be null on deployed contract");
    const content = jettonContentToCell({collection_content:"https://test-youtube.s3.amazonaws.com/manifest.json",common_content:"https://test-youtube.s3.amazonaws.com/manifest.json"});
 
    const res = await nftminterContract.sendNFT(sender,
                                              mintAddress, 
                                              parseInt(mintAmount),content);
    const gotTrans = await waitForTransaction(provider,
                                              nftminterContract.address,
                                              curState.lastTransaction.lt,
                                              10);
    if(gotTrans) {
...
export async function run(provider: NetworkProvider) {
    ...
    const minterCode = await compile('Jetton');
    let   done   = false;
    let   retry:boolean;
...
// 
    ...
    // nftminterContract = provider.open(NftCollection.createFromAddress(minterAddress));
