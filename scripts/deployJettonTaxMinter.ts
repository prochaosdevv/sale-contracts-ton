...
import { JettonMinterTax, JettonMinterContent, jettonContentToCell, jettonMinterConfigToCell } from '../wrappers/JettonTax';
...
export async function run(provider: NetworkProvider) {
    ...
    let contentUrl = await promptUrl(urlPrompt, ui, "https://test-youtube.s3.amazonaws.com/manifest.json");
    ...
    const minter  = JettonMinterTax.createFromConfig({admin,
                                                  content,
                                                  wallet_code,
                                                  }, 
                                                  await compile('JettonTax'));
