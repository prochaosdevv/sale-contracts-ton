...
export const promptUrl = async(prompt:string, ui:UIProvider, fallback?:string) => {
    let retry  = false;
    let input  = "";
    let res    = "";
    let promptFinal  = fallback ? prompt.replace(/:$/,'') + `(default:${fallback}):` : prompt ;
    do {
        input = await ui.input(promptFinal);
        try{
            let testUrl =  (input == "" && fallback ? new URL(fallback) : new URL(input));
